import"./style-F3p9moS5.js";var e=document.querySelector(`#app`);if(!e)throw Error(`Brak elementu #app`);var t=[],n=[],r=0,i=0,a=new Set,o=new Map,s=!1,c=[];async function l(){try{t=u(await(await fetch(`/Statystyka-Fiszki/data/bazy.json`)).json()),d(t)}catch(t){console.error(`Błąd przy ładowaniu pytań:`,t),e.innerHTML=`<p>Błąd przy ładowaniu pytań.</p>`}}function u(e){return e.map((e,t)=>{let n=typeof e.id==`number`?e.id:t+1;if(typeof e.correctAnswer==`number`)return{id:n,question:e.question,options:e.options,correctAnswer:e.correctAnswer};let r=e.options.findIndex(t=>t===e.correctAns);return r===-1&&console.warn(`Nie znaleziono poprawnej odpowiedzi dla pytania ${n}.`),{id:n,question:e.question,options:e.options,correctAnswer:r===-1?0:r}})}function d(e){n=[...e],r=0,i=0,a.clear(),o.clear(),s=!1,c=[],f()}function f(){if(!n.length)return;let t=n[r],c=a.has(t.id),l=o.get(t.id),u=t.options.map((e,n)=>{let r=`option`;return c&&(n===t.correctAnswer?r+=` correct`:n===l&&l!==t.correctAnswer&&(r+=` incorrect`)),`
        <button 
          type="button" 
      ...
      `}).join(``),d=`${r+1} / ${n.length}`,g=Math.round(i/n.length*100);e.innerHTML=`
    <div class="app">
      <header class="topbar">
        <div>
          <h1>
  ...
  `,document.querySelectorAll(`.option`).forEach(e=>{e.addEventListener(`click`,()=>{c||p(parseInt(e.getAttribute(`data-index`)))})});let _=document.getElementById(`prev-button`),v=document.getElementById(`next-button`),y=document.getElementById(`next-or-finish-button`);_?.addEventListener(`click`,()=>{r>0&&(r--,f())}),v?.addEventListener(`click`,()=>{r<n.length-1&&(r++,f())}),y?.addEventListener(`click`,()=>{r===n.length-1?h():(r++,f())}),document.getElementById(`shuffle-button`)?.addEventListener(`click`,m)}function p(e){let t=n[r];o.set(t.id,e),a.add(t.id),e===t.correctAnswer&&i++,f()}function m(){if(s=!s,s){c=[...n];let e=[...n];for(let t=e.length-1;t>0;t--){let n=Math.floor(Math.random()*(t+1));[e[t],e[n]]=[e[n],e[t]]}n=e}else n=c.length?[...c]:[...n];r=0,f()}function h(){let r=Math.round(i/n.length*100),a=n.filter(e=>o.get(e.id)!==e.correctAnswer),s=a.length,c=``;c=r===100?`🌟 Doskonale! Wszystkie odpowiedzi są poprawne!`:r>=80?`⭐ Świetnie! Wiele poprawnych odpowiedzi!`:r>=60?`👍 Dobrze. Warto powtórzyć kilka zagadnień.`:r>=40?`📚 Potrzebujesz więcej nauki. Spróbuj ponownie!`:`💪 Zacznij od początku i starannie czytaj pytania.`,e.innerHTML=`
    <div class="app">
      <header class="topbar">
        <div>
          <h1>
  ...
  `,document.getElementById(`restart-button`)?.addEventListener(`click`,()=>{d(t)}),document.getElementById(`retry-incorrect-button`)?.addEventListener(`click`,()=>{d(a)}),document.getElementById(`back-button`)?.addEventListener(`click`,()=>{window.location.href=`/Statystyka-Fiszki/`})}l();